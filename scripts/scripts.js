...
async function getJsonArray(url) {
    ...
    if (!Array.isArray(data)) {
        ...
        throw new TypeError("O formato de dados da API está incorreto. Esperada uma lista.");
    }
...
async function carregarCantores() {
    const selectCantores = document.querySelector(".AdicionarMusica .form-select");
    ...
    try {
        ...
        alert(`Não foi possível carregar a lista de cantores. Verifique a API. Detalhe: ${error.message}`);
    }
}
...
async function adicionarMusica(event) {
    ...
    const nomeArtista = document.querySelector(".input-group.artista input").value;
...
async function carregarMusicas() {
    ...
    try {
        ...
        musicas.forEach((musica) => {
            ...
            const colClasses = 'col-12 col-sm-6 col-lg-4 col-xl-3 mb-4 d-flex justify-content-center';
...
async function filtrarMusicasPorCantor(cantorId, cantorNome) {
    ...
    porCantoresSection.querySelectorAll('.row, h2.mt-3, p').forEach(el => el.remove());
